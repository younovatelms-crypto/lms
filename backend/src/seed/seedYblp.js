// src/scripts/seedYblp.js
//
// Push the full yblpCurriculum.js content into MongoDB.
//
// Default (SAFE) mode — enrich in place:
//   keeps the existing course _id, all subject _ids, statuses and your edits;
//   only fills the missing content fields (theory / practical / assignment +
//   lmsModuleId + order / days / weeks + month outcome / practicalSession).
//
//   node src/scripts/seedYblp.js                 # enrich existing YBLP
//   node src/scripts/seedYblp.js --dry           # preview, no writes
//   node src/scripts/seedYblp.js --fresh         # WIPE & re-create (new _ids!)
//   node src/scripts/seedYblp.js --uri="mongodb+srv://…"
//
// NOTE: requires the updated Course.js (with lmsModuleId / theoryContent /
// practicalActivity / assignmentTask on SubjectSchema, and outcome /
// practicalSession on MonthSchema) — otherwise strict mode drops the content.

require('dotenv').config();
const mongoose = require('mongoose');

const Course = require('../models/Course');
const { YBLP_COURSE, YBLP_TRIMESTERS } = require('../data/yblpCurriculum');
...
(async () => {
  ...
  const code = (arg('code', YBLP_COURSE.code) || 'YBLP').toUpperCase();
  ...
  if (FRESH) {
    if (DRY) {
      console.log(`· --fresh --dry: would delete course "${code}" and re-create it with`,
        YBLP_TRIMESTERS.reduce((a, t) => a + t.months.reduce((b, m) => b + m.subjects.length, 0), 0),
        'subjects.');
    } else {
      await Course.deleteOne({ code });
      const doc = await Course.create({ ...YBLP_COURSE, code, trimesters: YBLP_TRIMESTERS });
  ...
  const seedByNum = new Map(YBLP_TRIMESTERS.map((t) => [t.trimesterNumber, t]));
  let filled = 0;
  const missing = [];

  course.trimesters.forEach((t, ti) => {
    const sT = seedByNum.get(t.trimesterNumber) || YBLP_TRIMESTERS[ti];
    ...
    (t.months || []).forEach((m, mi) => {
      ...
      if (sM) {
        ...
        if (!m.outcome) m.outcome = sM.outcome || '';
        if (!m.practicalSession) m.practicalSession = sM.practicalSession || '';
      }
      ...
      (m.subjects || []).forEach((s, si) => {
        ...
        if (!s.lmsModuleId) s.lmsModuleId = sS.lmsModuleId || '';
  ...
  if (DRY) {
    ...
    console.dir({ name: sample?.name, lmsModuleId: sample?.lmsModuleId, status: sample?.status, theoryContent: sample?.theoryContent?.slice(0, 80) + '…' }, { depth: null });
  } else {
    await course.save();
