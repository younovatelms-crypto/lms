...
export default function TraineeDashboard() {
  ...
  const error = useAppSelector(selectTraineeError);
  ...
  if (status === 'failed') return <div style={{ padding: 32, color: '#B91C1C' }}>Error: {error}</div>;
  ...
  return (
    <div style={{ padding: 32, fontFamily: 'Calibri,sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 16, marginBottom: 32 }}>
        {[
          { label: 'Attendance %', value: `${data?.attendancePct ?? 0}%`, color: '#059669', icon: 'OK' },
          { label: 'Pending Work', value: data?.pendingAssignments, color: '#E94560', icon: 'W' },
          { label: 'Upcoming Sessions', value: data?.upcomingSessions?.length, color: '#4F46E5', icon: 'S' },
        ].map((s) => (
          <div key={s.label} style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', borderTop: `4px solid ${s.color}`, display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 2px 8px rgba(0,0,0,.06)' }}>
            ...
            <div>
              <p style={{ margin: 0, fontSize: 24, fontWeight: 700, color: s.color }}>{s.value ?? 0}</p>
              <p style={{ margin: 0, fontSize: 12, color: '#666' }}>{s.label}</p>
            </div>
          </div>
        ))}
      </div>
