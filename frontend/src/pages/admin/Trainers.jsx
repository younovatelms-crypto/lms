...
const Trainers = () => {
  ...
  // Per-trainer batch modal (mirrors Trainees exactly)
  ...
  // Set of batch-id strings a trainer currently belongs to.
  ...
  // [{ id, name }] for the batches a trainer is in (joined against the batches list).
    ...
    try {
      ...
      await Promise.all(
        chosenBatchIds.map((batchId) =>
          dispatch(
            assignTrainerToBatch({ trainerId: batchModalTrainer._id, batchId })
          ).unwrap()
        )
      );
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // DERIVED PAGINATION VALUES (after early returns)
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const currentSet     = batchModalTrainer ? currentBatchIdSet(batchModalTrainer) : new Set();
    ...
    <div style={{ padding: isMobile ? '16px 20px' : '32px 36px', fontFamily: 'Inter, system-ui, sans-serif', background: '#F8FAFC', minHeight: '100vh' }}>
      ...
      {trainers.length === 0 ? (
        <div style={{ background: '#ffffff', border: '1px solid #E2E8F0', borderRadius: 16, padding: isMobile ? 32 : 48, textAlign: 'center', boxShadow: '0 1px 3px rgba(15,23,42,0.06), 0 8px 24px rgba(30,58,95,0.08)' }}>
          <p style={{ color: '#64748B', fontSize: isMobile ? 13 : 14, margin: 0 }}>No trainers found.</p>
        </div>
      ) : (
        <div style={{ background: '#ffffff', border: '1px solid #E2E8F0', borderRadius: 16, boxShadow: '0 1px 3px rgba(15,23,42,0.06), 0 8px 24px rgba(30,58,95,0.08)', overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto', maxHeight: isMobile ? '320px' : '460px', overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: isMobile ? '620px' : '800px' }}>
              <thead style={{ position: 'sticky', top: 0, zIndex: 2 }}>
                <tr style={{ background: '#F8FAFC' }}>
                  <th style={thStyle}>#</th>
                  <th style={thStyle}>Name</th>
                  <th style={{ ...thStyle, display: window.innerWidth <= 480 ? 'none' : 'table-cell' }}>Email</th>
                  <th style={{ ...thStyle, display: window.innerWidth <= 600 ? 'none' : 'table-cell' }}>Specialization</th>
              ...
              <tbody>
                {paginatedTrainers.map((trainer, idx) => {
                  const trainerBatches = getTrainerBatches(trainer);
                  return (
                    <tr key={trainer._id || idx} style={{ borderBottom: '1px solid #E2E8F0' }}>
                      ...
                      <td style={{ padding: isMobile ? '8px 12px' : '12px 16px' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', padding: isMobile ? '2px 8px' : '3px 10px', borderRadius: 999, fontSize: isMobile ? 10 : 11, fontWeight: 700, background: trainer.isActive ? '#DCFCE7' : '#F1F5F9', color: trainer.isActive ? '#15803D' : '#475569' }}>
                          {trainer.isActive ? 'Active' : 'In-Active'}
                        </span>
                      </td>
                      ...
                      <td style={{ padding: isMobile ? '8px 12px' : '12px 16px', textAlign: 'right' }}>
                        <button
                          onClick={() => openBatchModal(trainer)}
      ...
      {/* ── PER-TRAINER BATCH MODAL with multi-select dropdown ── */}
      {batchModalTrainer && (
        <div
        ...
        >
          ...
          >
            ...
            <p style={{ color: '#64748B', fontSize: 14, marginBottom: 18 }}>
              {batchModalTrainer.name} — <span style={{ color: '#94A3B8' }}>{batchModalTrainer.email}</span>
            </p>
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              {addableBatches.length === 0 ? (
                <p style={{ fontSize: 13, color: '#94A3B8', margin: 0 }}>
                  This trainer is already in every batch.
                </p>
              ) : (
